...
class GATE(BaseModel):
    ...
    def train(self, X_train, y_train, params: Dict, extra_info: Dict):
        ...
        # DEBUG try defaults
        # self.model_config = GatedAdditiveTreeEnsembleConfig(task=self.task)
        ...
        tabular_model.fit(train=train, validation=validation, loss=self.loss_fn)
    ...
    def hyperopt_search(
        ...
        space = infer_hyperopt_space_gate(param_grid)
        ...
        def objective(params):
            ...
            # Split the train data into training and validation sets
            X_train, X_val, y_train, y_val = train_test_split(
                X, y, test_size=params["val_size"], random_state=self.random_state
            )

            data_config = DataConfig(
                target=["target"],
                continuous_cols=[
                    i for i in extra_info["num_col_names"] if i != "target"
                ],
            ...
            y_pred = tabular_model.predict(validation)[self.prediction_col].values
