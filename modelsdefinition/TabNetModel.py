...
class TabNetTrainer(BaseModel):
    ...
    def prepare_tabular_model(self, params, outer_params, default=False):
        ...
        model_config = TabNetModelConfig(task=self.task, **valid_params)
    ...
    def hyperopt_search_kfold(
        ...
        def objective(params):
            ...
            self.logger.debug(
                f"Batch Size {params['batch_size']} | VBS {params['batch_size']*params['virtual_batch_size_ratio']}"
            )

            # Loop through each element in the list
            for fold_length in possible_fold_sizes:
                if fold_length % params["virtual_batch_size"] == 1:
                    self.logger.debug(
