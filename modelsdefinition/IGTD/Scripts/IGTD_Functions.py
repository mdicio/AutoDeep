...
def min_max_transform(data: pd.DataFrame, exclude_cols: List, feature_range=(0, 255)):
    ...
    Input:
    data: an input data frame with a size of [n_sample, n_feature]
    Return:
    data: the data frame after transformation
    """
...
def IGTD_absolute_error(
    ...
    print(save_folder)
    # print(type(index_record), type(err_record), run_time)
...
def generate_image_data(
    ...
    save_mode: In normal mode, each image is saved independently, if any other mode is specified, images are saved in bulk (one file for all images).
    save_pngs: If True, pngs of images are saved.
