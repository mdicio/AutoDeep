...
class IGTDPreprocessor:
    ...
    def __init__(
        self, dataset_name: str = None, igtd_configs: Dict[str, Dict] = None, base_result_dir: str = 'igtd', exclude_cols: Optional[List[str]] = None
    ):
        """__init__
...
class ExtraInfoCreator:
    ...
    def __init__(self, dataset_name, run_igtd=False, igtd_preprocessor: Optional[IGTDPreprocessor] = None):
        """__init__
    ...
    def create_extra_info(self, df: pd.DataFrame, dataset_name: str):
        ...
        if self.run_igtd and self.igtd_preprocessor:
            ...
            elif self.igtd_preprocessor.img_rows is None or self.igtd_preprocessor.img_columns is None:
                (self.igtd_preprocessor.img_rows, self.igtd_preprocessor.img_columns) = self.igtd_preprocessor._auto_determine_img_size(df)
...
class DataLoader:
    ...
    def bin_random_numerical_column(self, train_df, test_df, exclude_cols, bins=10):
        ...
        train_df[col_to_bin], bin_values = pd.cut(
            train_df[col_to_bin], bins=bins, labels=[i for i in range(bins)], include_lowest=True, right=True, retbins=True
        )
        test_df[col_to_bin] = pd.cut(test_df[col_to_bin], bins=bin_values, labels=[i for i in range(bins)], include_lowest=True, right=True)
...
class DynamicDataLoader(DataLoader):
    ...
    def __init__(
        ...
        self.extra_info_creator = ExtraInfoCreator(
            ...
            igtd_preprocessor=(
                IGTDPreprocessor(dataset_name=self.dataset_name, igtd_configs=self.igtd_configs, base_result_dir=self.igtd_result_base_dir)
                if self.run_igtd
                else None
            ),
        )
    ...
    def load_data(self):
        ...
        if self.split_col and self.split_col in df.columns:
            ...
            X_train, X_test, y_train, y_test = train_test_split(
                X, y, test_size=self.test_size, random_state=self.random_state, stratify=y if self.problem_type != 'regression' else None
            )
...
class KaggleAgeConditionsLoader(DataLoader):
    ...
    def __init__(
        self, target_column='target', test_size=0.2, random_state=42, normalize_features='mean_std', return_extra_info=False, encode_categorical=False
    ):
        """__init__
    ...
    def load_data(self):
        ...
        df_train, df_test = train_test_split(df, test_size=self.test_size, random_state=self.random_state, stratify=df[self.target_column])
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=8, img_columns=7, igtd_path=f'{self.igtd_path}results/ageconditions_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
...
class BufixDataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=10, img_columns=11, igtd_path=f'{self.igtd_path}results/bufix_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
...
class TitanicDataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=4, img_columns=7, igtd_path=f'{self.igtd_path}results/titanic_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
...
class BreastCancerDataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=6, img_columns=5, igtd_path=f'{self.igtd_path}results/breastcancer_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
...
class CreditDataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=5, img_columns=6, igtd_path=f'{self.igtd_path}results/creditcard_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
...
class IrisDataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=2, img_columns=2, igtd_path=f'{self.igtd_path}results/iris_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
...
class CaliforniaHousingDataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=3, img_columns=3, igtd_path=f'{self.igtd_path}results/housing_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
...
class AdultDataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=9, img_columns=12, igtd_path=f'{self.igtd_path}results/adult_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
...
class CoverTypeDataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=6, img_columns=9, igtd_path=f'{self.igtd_path}results/covertype_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
...
class HelocDataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        if self.return_extra_info:
            extra_info = self.create_extra_info(
                X_train, img_rows=6, img_columns=4, igtd_path=f'{self.igtd_path}results/heloc_igtd_Euclidean_Euclidean/abs/_index.txt'
            )
        return X_train, X_test, y_train, y_test, extra_info
