...
class SoftOrdering1DCNN:
    ...
    def _pandas_to_torch_datasets(self, X_train, y_train, val_size, batch_size):
        ...
        y_train_tensor = torch.tensor(y_train.values, dtype=torch.float if self.problem_type == 'regression' else torch.long)
    ...
    def _set_optimizer_schedulers(self, params, default_params: Optional[Dict] = None):
        ...
        if optimizer_fn_name == 'Adam':
            self.optimizer = optim.Adam(self.model.parameters(), lr=optimizer_params['learning_rate'], weight_decay=optimizer_params['weight_decay'])
        elif optimizer_fn_name == 'SGD':
            self.optimizer = optim.SGD(self.model.parameters(), lr=optimizer_params['learning_rate'], momentum=optimizer_params['momentum'])
        elif optimizer_fn_name == 'AdamW':
            self.optimizer = optim.AdamW(self.model.parameters(), lr=optimizer_params['learning_rate'], weight_decay=optimizer_params['weight_decay'])
        if scheduler_fn_name == 'StepLR':
            self.scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=scheduler_params['step_size'], gamma=scheduler_params['gamma'])
            ...
            self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(
                self.optimizer, factor=scheduler_params['factor'], patience=scheduler_params['patience'], min_lr=1e-07, verbose=True, mode='min'
            )
    ...
    def hyperopt_search(self, X, y, model_config, metric, eval_metrics, max_evals=16, extra_info=None):
        ...
        def objective(params):
            ...
            train_loader = torch.utils.data.DataLoader(
                self.torch_dataset_train, batch_size=bs, shuffle=True, drop_last=False, num_workers=self.num_workers, pin_memory=True
            )
            val_loader = torch.utils.data.DataLoader(
                self.torch_dataset_val, batch_size=bs, shuffle=False, drop_last=False, num_workers=self.num_workers, pin_memory=True
            )
