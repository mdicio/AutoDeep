    ...
    def hyperopt_search(self, X, y, model_config, metric, eval_metrics, max_evals=16, extra_info=None):
        ...
        def objective(params):
            ...
            if self.problem_type == 'regression':
                model = MLPRegressor(verbose=False, early_stopping=True, n_iter_no_change=n_iter_no_change, max_iter=max_iter, **filtered_params)
            elif self.problem_type in ['binary_classification', 'multiclass_classification']:
                model = MLPClassifier(verbose=False, early_stopping=True, n_iter_no_change=n_iter_no_change, max_iter=max_iter, **filtered_params)
            else:
