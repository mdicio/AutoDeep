...
def map_scheduler_str_to_class(scheduler_str):
    ...
    Returns:
        type: Description
    """
    scheduler_mapping = {'StepLR': StepLR, 'ExponentialLR': ExponentialLR, 'ReduceLROnPlateau': ReduceLROnPlateau}
...
def prepare_optimizer(optimizer_fn):
    ...
    if isinstance(optimizer_fn, dict):
        ...
        if optimizer_fn == Adam:
            return 'Adam', {'weight_decay': optimizer_details.get('Adam_weight_decay', 0.0)}, optimizer_details.get('Adam_learning_rate', 0.001)
        elif optimizer_fn == SGD:
            return (
                'SGD',
                {'weight_decay': optimizer_details.get('SGD_weight_decay', 0.0), 'momentum': optimizer_details.get('SGD_momentum', 0.0)},
                optimizer_details.get('SGD_learning_rate', 0.001),
            )
        elif optimizer_fn == AdamW:
            return 'AdamW', {'weight_decay': optimizer_details.get('AdamW_weight_decay', 0.01)}, optimizer_details.get('AdamW_learning_rate', 0.001)
    return None, {}
...
def prepare_scheduler(scheduler_fn):
    ...
    if isinstance(scheduler_fn, dict):
        ...
        if scheduler_fn == StepLR:
            return 'StepLR', {'step_size': scheduler_details.get('StepLR_step_size', 10), 'gamma': scheduler_details.get('StepLR_gamma', 0.1)}
        elif scheduler_fn == ExponentialLR:
            return 'ExponentialLR', {'gamma': scheduler_details.get('ExponentialLR_gamma', 0.9)}
                    ...
                    [
                        {
                            ...
                            **{
                                f'{opt_name}_{sub_param}': (
                                    ...
                                    else scope.int(hp.quniform(f'{opt_name}_{sub_param}', *ensure_min_max(param_range), 1))
                                )
                    ...
                    [
                        {
                            ...
                            **{
                                f'{sched_name}_{sub_param}': (
                                    ...
                                    if isinstance(param_range[0], float)
                                    else scope.int(hp.quniform(f'{sched_name}_{sub_param}', *ensure_min_max(param_range), 1))
                                )
...
def infer_hyperopt_space_pytorch_tabular_old1(param_grid: Dict):
    ...
    for param_name, param_values in param_grid.items():
        ...
        if isinstance(param_values, dict):
            ...
            if param_name == 'optimizer_fn':
                space[param_name] = hp.choice(param_name, [map_optimizer_str_to_class(i) for i in param_values.keys()])
            if param_name == 'scheduler_fn':
                space[param_name] = hp.choice(param_name, [map_scheduler_str_to_class(i) for i in param_values.keys()])
            for sparam, svalue in param_values.items():
                print(sparam, svalue)
                for subsparam, subvalue in svalue.items():
            ...
            isinstance(param_values[0], (str, bool, list))
            or param_name in ['virtual_batch_size_ratio', 'weights', 'input_embed_dim_multiplier', 'hidden_size']
            or any(value is None for value in param_values)
        ):
            print('yo', param_name)
            if param_name in ['weights']:
