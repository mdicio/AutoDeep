...
def load_example_data():
    ...
    Returns:
        type: Description
    """
    res = pd.read_csv('../Data/Example_Drug_Response_Data.txt', sep='\t', engine='c', na_values=['na', '-', ''], header=0, index_col=None)
    ...
    for i in range(len(files)):
        if files[i].split('.')[1] == 'txt' and files[i].split('_')[0] == 'Drug':
            id.append(i)
            data = pd.read_csv(
                '../Data/Example_Drug_Descriptor_Image_Data/' + files[i], sep='\t', engine='c', na_values=['na', '-', ''], header=None, index_col=None
            )
    ...
    for i in range(len(files)):
        if files[i].split('.')[1] == 'txt' and files[i].split('_')[0] == 'CCL':
            id.append(i)
            data = pd.read_csv(
                '../Data/Example_Gene_Expression_Image_Data/' + files[i], sep='\t', engine='c', na_values=['na', '-', ''], header=None, index_col=None
            )
            image[i, :, :, 0] = data.values
            sample.append(files[i].split('.txt')[0])
    image = image[id, :, :, :]
...
class CNN2D_Regressor:
    ...
    def __init__(self, params, input_data_dim, dropout):
        ...
        for input_id in range(num_input):
            in_id = Input(shape=(self.input_data_dim[input_id][0], self.input_data_dim[input_id][1], 1), name='Input_' + str(input_id))
            input.append(in_id)
            for j in range(num_kernel_size):
                ...
                for i in range(num_conv_layer[j]):
                    ...
                    d = BatchNormalization(axis=-1, name='BatchNorm_' + str(i) + '_Kernel_' + str(j) + '_Input_' + str(input_id))(inputs=d)
                    ...
                    d = MaxPooling2D(
                        pool_size=self.params['pool_size'][j], name='MaxPooling_' + str(i) + '_Kernel_' + str(j) + '_Input_' + str(input_id)
                    )(d)
        ...
        for i in range(num_dense_layer):
            if self.params['activation'] == 'selu':
                d = Dense(
                    self.params['network_layers'][i], activation=self.params['activation'], name='Dense_' + str(i), kernel_initializer='lecun_normal'
                )(d)
            else:
...
def CNN2D_Regression_Analysis(train, resultFolder, para, val=None, test=None):
    ...
    for i in ['train', 'val']:
        ...
        perM[i].fill(np.inf)
        perM[i] = pd.DataFrame(
            perM[i], index=[('dropout_' + str(j)) for j in para['drop']], columns=[('epoch_' + str(j)) for j in range(para['epochs'])]
        )
    for dpID in range(len(para['drop'])):
        ...
        model_saver = ModelCheckpoint(
            resultFolder + '/model_dropout_' + str(para['drop'][dpID]) + '.h5', monitor=monitor, save_best_only=True, save_weights_only=False
        )
        ...
        early_stop = EarlyStopping(monitor=monitor, patience=para['es_patience'], min_delta=para['es_min_delta'], verbose=1)
    ...
    perf = np.empty((3, 7))
    perf.fill(np.nan)
    perf = pd.DataFrame(perf, columns=['R2', 'MSE', 'MAE', 'pCor', 'pCorPvalue', 'sCor', 'sCorPvalue'], index=['train', 'val', 'test'])
...
def CNN2D_Classification_Analysis(train, num_class, resultFolder, para, class_weight=None, val=None, test=None):
    ...
    if test is not None:
        predResult['test'] = {}
        predResult['test']['proba'] = pd.DataFrame(model.predict(testData), index=testSample, columns=[('proba_' + str(i)) for i in range(num_class)])
        predResult['test']['label'] = pd.DataFrame(
            np.argmax(a=predResult['test']['proba'].values, axis=1), index=predResult['test']['proba'].index, columns=['prediction']
        )
    predResult['train'] = {}
    predResult['train']['proba'] = pd.DataFrame(model.predict(trainData), index=trainSample, columns=[('proba_' + str(i)) for i in range(num_class)])
    predResult['train']['label'] = pd.DataFrame(
        np.argmax(a=predResult['train']['proba'].values, axis=1), index=predResult['train']['proba'].index, columns=['prediction']
    )
    if val is not None:
        predResult['val'] = {}
        predResult['val']['proba'] = pd.DataFrame(model.predict(valData), index=valSample, columns=[('proba_' + str(i)) for i in range(num_class)])
        predResult['val']['label'] = pd.DataFrame(
            np.argmax(a=predResult['val']['proba'].values, axis=1), index=predResult['val']['proba'].index, columns=['prediction']
        )
    backend.clear_session()
    perf = np.empty((3, 3))
    perf.fill(np.nan)
    ...
    for k in ['train', 'val', 'test']:
        ...
        if num_class == 2:
            ...
            perf.loc[k, 'AUROC'] = roc_auc_score(
                keras.utils.to_categorical(eval(k + 'Label')), predResult[k]['proba'].values, labels=range(num_class), multi_class='ovr'
            )
